...
- **File-based storage with file locking.** All graph and event data is persisted to local files (`graph.json`, `events.jsonl`). Concurrent access is coordinated via file locks (cross-platform via `filelock`), which are effective only on a single host.
...
| Limitation | Details |
| --- | --- |
| Single-host file locking | File locks are local to a single machine. Running multiple Cascade instances against shared networked storage (e.g., NFS) is not safe. |
